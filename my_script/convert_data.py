...
def level_traversal(seq_id, tree, out_dic, real, out_order_list, merged_dic, ncbi):
    ...
    try:
        ...
        os.system(f"echo '{str(e)}>>>>>>>>>>{seq_id}' >> error")
    return None
...
def func(index, lines_pool):
    ...
    for weight, out_order_list, adjacent_matrix, Y in handle_data(lines_pool, merged_dic, ncbi):
        ...
        Ys.append(Y)
    np.save(f"./out_data/x1_{index}", x1)
    np.save(f"./out_data/x2_{index}", x2)
    np.save(f"./out_data/adjacent_matrixs_{index}", adjacent_matrixs)
    np.save(f"./out_data/Y_{index}", Ys)


lines = open("/data/home/wlzhang/classfication/kraken2_db/2021_11_01/library/stimulation/test/1.fastq.origin.output.typed", "r").readlines()
processing_nums = 100
...
for index, lines_pool in enumerate(data_iter(processing_nums, lines)):
    ...
    pool.apply_async(func, (index, lines_pool,))
pool.close()
pool.join()
...
# adjacent_matrixs = []
# Ys = []
# for index in range(processing_nums):
#     x1.extend(np.load(f"./out_data/x1_{index}.npy", allow_pickle=True))
#     x2.extend(np.load(f"./out_data/x2_{index}.npy", allow_pickle=True))
#     adjacent_matrixs.extend(
#         np.load(f"./out_data/adjacent_matrixs_{index}.npy", allow_pickle=True))
#     Ys.extend(np.load(f"./out_data/Y_{index}.npy", allow_pickle=True))
#     print(index)
# np.save("x1", x1)
# np.save("x2", x2)
# np.save("adjacent_matrixs", adjacent_matrixs)
# np.save("Y", Ys)
