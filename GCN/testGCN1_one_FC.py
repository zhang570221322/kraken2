...
arg = Arg(100, 0.000942607974725336, 0.75, 10)
...
output_dim_1 = len(dataset.level_dim)
output_dim_2 = max(dataset.level_dim.values())
output_dim =   sum(dataset.level_dim.values())
# pdb.set_trace()
...
weight = torch.Tensor([10,10,10,10,10,10,10]).to(device)
...
def train():
    ...
    for data in train_loader:
        ...
        # pdb.set_trace()
        reshape_len = int(len(label)/output_dim_1)
        label = label.reshape(reshape_len,  output_dim_1)
        loss=model.criterion(crit,output,label,dataset.level_dim,weight=weight)
        loss.backward()
        loss_all += loss.item()
        optimizer.step()
        optimizer.zero_grad()
...
def evaluate(loader):
    ...
    with torch.no_grad():
        for data in loader:
            data = data.to(device)
            pred = model(data)
            label = data.tax3.detach().cpu().numpy()
    ...
    for batch in predictions:
        ...
        for i in list(dataset.level_dim.values())[::-1]:
            end+=i
            prdi_index = batch[:,start:end].argmax(axis=-1).detach().cpu().numpy()
            temp.append(prdi_index)
            start+=i
        for one_data in np.array(temp).T:
            label= get_tax_list(dataset.level_tax,one_data,mode=2)[level_class]
...
torch.save(model.state_dict(), "./args/dp1_2_k3")
