            ...
            for i in range(1, 30):
                ...
                if end + i <= len(read.seq):
                    set_value(adj_matrix, feature_index,
                              read_kmers, start+i, i, feature_space)
        read_kmers.clear()
    return [temp[1] for temp in list(feature_space.values())], sparse.csc_matrix(adj_matrix), tax_id
...
def func(index, files):
    ...
    for file_name in files:
        ...
        Ys.append(y)
    np.save(f"{our_dir}/out_data2/x_{index}", feature)
    np.save(f"{our_dir}/out_data2/adjacent_matrixs_{index}", adjacent_matrixs)
    np.save(f"{our_dir}/out_data2/Y_{index}", Ys)


our_dir = "/data/home/wlzhang/classfication/software/kraken2/test/stimulation_test/dp2/data"
